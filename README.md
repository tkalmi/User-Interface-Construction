...
Instead of using the regular `create-react-app`, this version utilises
`TypeScript` as well. It was created with the command `create-react-app
ui-construction --scripts-version=react-scripts-ts` per the instructions found
<a href="https://github.com/wmonk/create-react-app-typescript">here</a>.

### Hot Module Replacement (HMR)

Instead of settling for live reloading while developing, this project uses
<i>hot module replacement</i> (HMR for short), which basically replaces only the
changed parts of the application without messing up the app state (there are
some limitations though). This was done using `react-app-rewired`, more
specifically
<a href="https://github.com/Arelav/react-app-rewire-typescript-hmr">this hobby
project</a> (I know there aren't any stars or followers, but it seems to work
pretty well, which is enough for the scope of the UI construction project)

### Automatic `prettier` on Git commits

This project uses `prettier` for code formating. Prettier is ran automatically
every time `git commit` is called. This is achieved with `husky` and
`lint-staged`, and by adding `"precommit": "lint-staged"` in `package.json`'s
`"scripts"` along with a specific `"lint-staged"` field.

### Material Design

As we're using Google's material design guidelines for our project,
<a href="https://material-ui-next.com/">`material-ui`</a> is installed. There's
also material design's default font, `Roboto`, installed with the `npm` package
`typeface-roboto`.

Below you'll find the standard `create-react-app` auto-generated documentation.
